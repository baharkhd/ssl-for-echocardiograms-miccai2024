...
from utils.video_transforms import RandomRotateVideo

# filter out pytorch user warnings for upsampling behaviour
warnings.filterwarnings("ignore", category=UserWarning)

class_labels = ["No AS", "Early AS", "Significant AS"]
as_tom_path = "/mnt/nas-server/published/vaseli_ProtoASNet_MICCAI2023/data_removelater/as_tom/"
...
class AorticStenosisDataset(Dataset):  # TODO modify to your own dataset
    ...
    def _get_item_from_info(self, data_info, window_start, window_end, interval_idx=0):
        ...
        """
        cine_original = loadmat(os.path.join(as_tom_path, data_info["path"]))["cine"]  # T_original xHxW
        ...
        diagnosis_label_to_class_mapping = {'normal':0, 'mild':1, 'moderate': 2, 'severe':3, 'unassigned': 4}
        label_as = torch.tensor(diagnosis_label_to_class_mapping[data_info["as_label"]])
...
if __name__ == "__main__":
    data_config = {   # TODO modify to your own dataset
        ...
        "name": "as_tom",
        "data_info_file": "/mnt/nas-server/published/vaseli_ProtoASNet_MICCAI2023/data_removelater/as_tom/annotations-all.csv",
        "sample_size": None,
    ...
    for sample_dict in dataloader:
        print(f"target_AS: {sample_dict['target_AS']}")
        print(f"Label shape : {sample_dict['target_AS'].shape}")
        print(f"filenames : {sample_dict['filename']}")
        print(f"cine shape: {sample_dict['cine'].shape}")
        print()
